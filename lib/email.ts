...
import type { EventDefinition } from "./config";
...
export async function sendTicketEmail(
  ...
  event: Pick<EventDefinition, "date" | "location">,
) {
  if (!isMailConfigured()) {
  ...
  await transporter.sendMail({
    ...
    subject: `Je ticket voor ${registration.eventName}`,
    text: [
      `Hallo ${registration.name},`,
      "",
      `Je inschrijving voor ${registration.eventName} is bevestigd.`,
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #d8dee8;border-radius:16px;">
          <tr>
            <td style="padding:32px 28px 16px;">
              ...
              <h1 style="margin:0 0 12px;font-size:28px;line-height:1.2;font-weight:700;">${escapeHtml(registration.eventName)}</h1>
          ...
          <tr>
            <td style="padding:0 28px 28px;font-size:14px;line-height:1.6;color:#5c6b80;">
              <p style="margin:0 0 6px;"><strong style="color:#081C3C;">Event</strong> ${escapeHtml(registration.eventName)}</p>
