      ...
      <div className="flex flex-col gap-8 p-7 sm:p-10">
        ...
        {cancelled ? (
          <p className="rounded-2xl border border-accent/30 bg-accent/10 px-4 py-3 text-sm text-accent">
            Deze inschrijving is geannuleerd. Het ticket is niet meer geldig voor
            check-in.
          </p>
        ) : (
        ...
        {!cancelled && isNew && emailSent === false ? (
          ...
          <p className="no-print text-sm text-muted">
            Je ticket staat ook in je mailbox (met een link om je uit te schrijven).
            Toon deze QR-code aan de ingang.
          </p>
        ) : null}
        <div className="no-print flex flex-wrap gap-3">
          ...
          {!cancelled ? (
            <Link
              href={`/uitschrijven/${encodeURIComponent(registration.ticketToken)}`}
